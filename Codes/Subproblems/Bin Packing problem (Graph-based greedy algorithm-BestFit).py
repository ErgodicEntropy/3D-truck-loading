...
#Luck condition:
#Strong condition: B[k] - sum1-h(S[j]) >= sum1-m(B[i]), i != k
#Weak condition: B[k] - sum1-h(S[j]) >= B[i] for all i in [|1,m|]/{k}

#h is the persistance/degree of luck (extent to which greedy algo = dynamic prog). Thus best-case scenario <=> h = n, worst-case scenario <=> h = 1
